...
function buildNav(members) {
    ...
    nav += buildMemberNav(members.modules, 'Suites List', {}, linkto);
    nav += buildMemberNav(members.externals, 'Externals', seen, linktoExternal);
    nav += buildMemberNav(members.namespaces, 'Namespaces', seen, linkto);
    nav += buildMemberNav(members.classes, 'Tests List', seen, linkto);
    ...
    if (members.globals.length) {
        ...
        if (!globalNav) {
            ...
            nav += `<h3>Commands</h3><ul>${globalNav}</ul>`;
        }
    }
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    if (members.globals.length) { generate('Commands', [{kind: 'globalobj'}], globalUrl); }
    ...
    Object.keys(helper.longnameToUrl).forEach(longname => {
        ...
        if (myClasses.length) {
            generate(`Test: ${myClasses[0].name}`, myClasses, helper.longnameToUrl[longname]);
        }
